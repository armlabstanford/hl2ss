...
if __name__ == '__main__':
    ...
    while (enable):
        ...
        color = data_pv.payload.image # RGB, HxW
        color = cv2.cvtColor(color, cv2.COLOR_BGR2RGB) # opencv bgr to rgb

        # RGB to Depth (from Depth-Anything) ---------------------------------------------------
        depth_output = model.infer_image(color) # HxW raw depth map in numpy
        normalized_depth = cv2.normalize(depth_output, None, 0, 255, cv2.NORM_MINMAX) # Normalize the depth map for display
        normalized_depth = normalized_depth.astype('uint8')
        depth_anything = cv2.applyColorMap(normalized_depth, cv2.COLORMAP_JET) # DEPTH, HxW
        ...
        depth_remapped    = cv2.remap(depth_anything, pv_uv[:, :, 0], pv_uv[:, :, 1], cv2.INTER_LINEAR)
        ...
        image = np.hstack((hl2ss_3dcv.rm_depth_to_rgb(depth) / 8, depth_remapped / 255)) # Depth scaled for visibility
        cv2.imshow('DD', image)
